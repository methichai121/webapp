...
const Activitydetailteacher = () => {
  ...
  // ฟังก์ชันติดตามกิจกรรม
  const handleFollow = () => {
    let followed =
      JSON.parse(localStorage.getItem("followedActivities")) || [];

    if (!followed.find((a) => a.activity_id === activity.activity_id)) {
      followed.push(activity);
      localStorage.setItem("followedActivities", JSON.stringify(followed));
        ...
        <div style={styles.mainContent}>
          ...
          <div style={styles.leftSide}>
            <nav style={styles.nav}>
              <Link to="/hometeacher" className="nav-item">
                หน้าหลัก
              </Link>
              <Link to="/profileteacher" className="nav-item">
                โปรไฟล์
              </Link>
              <Link to="/searchactivityteacher" className="nav-item">
                ค้นหากิจกรรม
              </Link>
              <Link to="/followactivityteacher" className="nav-item">
                กิจกรรมที่ติดตาม
              </Link>
              <Link to="/addactivity" className="nav-item">
                เพิ่มกิจกรรม
              </Link>
              <Link
                to="/"
