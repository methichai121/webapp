...
function Editprofileteacher() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    axios.get(`http://localhost:5000/api/teacher/${userId}`)
      .then(res => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `http://localhost:5000/api/teacher/update/${userId}`,
        payload,
  ...
  return (
    <div>
      ...
      <div className="top-left">
        <Link to={`/profileteacher/`} className="link-button">
      ...
      <div className="top-right">
        <Link to="/hometeacher" className="link-button">
